...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ApiData {
    /// Gradio 5.x: string; 6.x: i18n map { key, _type } —— 兼容两者 (map 取 key)
...
/// Gradio 6.x 把参数 label 包成 i18n map ({ "key": ..., "_type": "translation_metadata" }),
/// 5.x 仍是纯字符串 —— 两种形态都接受, 归一化为 Option<String>。
    ...
    D: serde::Deserializer<'de>,
{
    ...
    Ok(match v {
        ...
        Some(serde_json::Value::Object(m)) => m.get("key").and_then(|k| k.as_str()).map(String::from),
