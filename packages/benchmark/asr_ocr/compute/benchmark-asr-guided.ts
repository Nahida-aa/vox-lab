...
if (require.main === module) {
  ...
  run(
    "ocr-asr-guided-end",
    endSegs,
    `ASR end time (${endOcrCount} OCR calls, ${endElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    endOcrCount,
    endElapsed,
  ...
  run(
    "ocr-asr-guided-mid-so",
    soSegs,
    `ASR midpoint + subtitle-only (${soOcrCount} OCR calls, ${soElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    soOcrCount,
    soElapsed,
  ...
  run(
    "ocr-asr-guided-fps1",
    fpsSegs,
    `ASR 1fps intervals (${fpsOcrCount} OCR calls, ${fpsElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    fpsOcrCount,
    fpsElapsed,
  ...
  run(
    `ocr-asr-guided-midfps-${ASR_SOURCE}-${ASR_PARAM}`,
    midFpsSegs,
    `ASR midpoint ±1fps clamped (${midFpsOcrCount} OCR calls, ${midFpsElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    midFpsOcrCount,
    midFpsElapsed,
