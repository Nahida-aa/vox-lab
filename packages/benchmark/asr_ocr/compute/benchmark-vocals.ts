...
function run(
  ...
  // Write ASR source
  writeFileSync(
    join(resultDir, "metadata", "asr.json"),
    JSON.stringify(
      {
        _engine: "whisper.cpp",
  ...
  console.log(
    `  CER=${(cerDetail.cer * 100).toFixed(2)}% S=${cerDetail.char_subs} I=${cerDetail.char_ins} D=${cerDetail.char_dels}`,
  );
...
if (require.main === module) {
  ...
  const frameFiles = readdirSync(allFramesDir)
  ...
  run(
    "sep-ocr-fixed-1fps",
    fixedResults,
    `Fixed 1fps (${fixedOcrCount} OCR calls, ${fixedElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    fixedOcrCount,
    fixedElapsed,
  ...
  run(
    "sep-ocr-guided-mid",
    mergeSegments(midResults),
    `ASR midpoint (${ocrCount} OCR calls, ${midElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    ocrCount,
    midElapsed,
  ...
  run(
    "sep-ocr-guided-end",
    mergeSegments(endResults),
    `ASR end (${endOcrCount} OCR calls, ${endElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    endOcrCount,
    endElapsed,
  ...
  run(
    "sep-ocr-guided-mid-so",
    mergeSegments(soResults),
    `ASR midpoint (${soOcrCount} OCR calls, ${soElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    soOcrCount,
    soElapsed,
  ...
  run(
    "sep-ocr-guided-fps1",
    mergeSegments(fpsSegs, 200),
    `ASR 5fps intervals (${fpsOcrCount} OCR calls, ${fpsElapsed.toFixed(1)}s)`,
    gtSegs,
    asrSegs,
    fpsOcrCount,
    fpsElapsed,
  ...
  console.log(
    "label                      | segs | hyp_ch | ref_ch | CER%   | S  | I  | D  | recall_seg | recall_ch",
  );
  console.log(
    "---------------------------|------|--------|--------|--------|----|----|----|------------|-----------",
  );
  ...
  for (const label of labels) {
    ...
    console.log(
      `${l} | ${seg} | ${hc} | ${rc} | ${cer}% | ${sub} | ${ins} | ${del} | ${rs} | ${rc2}`,
    );
  }
}
